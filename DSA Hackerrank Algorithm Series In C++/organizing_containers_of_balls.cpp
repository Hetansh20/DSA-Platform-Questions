...
string organizingContainers(vector<vector<int>> container) {
    ...
    vector<long long> capacity(n, 0), balls(n, 0);

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            capacity[i] += container[i][j];  // total in container i
            balls[j] += container[i][j];     // total of type j
        }
    }
    ...
    return (capacity == balls) ? "Possible" : "Impossible";
}
