...
int luckBalance(int k, vector<vector<int>> contests) {
    int totalLuck = 0;
    vector<int> importantContestsLuck; 
    ...
    for (const auto& contest : contests) {
        ...
        if (T == 0) {
            ...
            importantContestsLuck.push_back(L);
        }
    }

    sort(importantContestsLuck.rbegin(), importantContestsLuck.rend());

    int numImportant = importantContestsLuck.size();

    for (int i = 0; i < numImportant; ++i) {
        if (i < k) {
            totalLuck += importantContestsLuck[i];
        } else {
            totalLuck -= importantContestsLuck[i];
        }
    }
